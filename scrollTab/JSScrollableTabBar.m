...
- (id)initWithFrame:(CGRect)frame style:(JSScrollableTabBarStyle)style
{
    if ((self = [super initWithFrame:frame]))
	{
		...
		_background = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 44)];
		[_background setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
		[self addSubview:_background];
		
		_scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0,0, 320, 44)];
...
-(JSTabItem *) getTabItemsByIndex:(int) index
{
...
- (void)layoutTabs
{
    ...
    NSLog(@"with ====== %f,size = %f,%@",overallWidth+padding,_scrollView.contentSize.width,_scrollView
          );
